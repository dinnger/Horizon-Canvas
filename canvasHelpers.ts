...
export function render_node({
	...
	ctx.fillText(
		node.info.icon,
		node.design.x + node.design.width! / 2 - 20,
		node.design.y + 50,
	);
	// name
	...
	ctx.fillText(
		node.info.name,
		node.design.x + node.design.width! / 2,
		node.design.y + 65,
		700,
	);
	// info
	if (infoTrace.inputs > 0 || infoTrace.outputs > 0) {
		...
		ctx.fillText(
			`${infoTrace.inputs}/${infoTrace.outputs}`,
			node.design.x + node.design.width! - 10,
			node.design.y + 18,
		);
	}
	...
	for (const [type, connector] of Object.entries(node.info.connectors)) {
		...
		for (const [key, input] of Object.entries(connector)) {
			...
			if (type === "callbacks") {
				...
				x =
					node.design.x +
					(node.design.width! - 5) / 2 -
					totalWidth / 2 +
					Number.parseInt(key) * spacing;
...
export function addAnimation({
	...
	for (const connection of node.connections.filter(
		(f) =>
			f.idNodeOrigin === node.id &&
			f.connectorType === conn.type &&
			f.connectorName === conn.name,
	)) {
...
export function renderSelected({
	...
	ctx.roundRect(
		canvasSelect.x1,
		canvasSelect.y1,
		canvasSelect.x2 - canvasSelect.x1,
		canvasSelect.y2 - canvasSelect.y1,
		10,
	);
	ctx.stroke();
	ctx.setLineDash([]);
...
export function drawNodeConnectionPreview({
	...
	draw_bezier({
		...
		destinyColor:
			hasValidConnection && node_destiny?.node
				? node_destiny.node.info.color
				: undefined,
	});
	...
	if (
		...
		ctx.beginPath();
		ctx.strokeStyle = node_destiny?.node
			? node_destiny.node.info.color
			: node_origin.info.color;
		ctx.lineWidth = 5;
		ctx.shadowColor = node_destiny?.node
			? node_destiny.node.info.color
			: node_origin.info.color;
		ctx.shadowBlur = 10;
		const inputX = node_destiny.node.design.x - 7;
		const inputY =
			node_destiny.node.design.y + 25 + node_destiny.input_index * 20 - 1;
...
export function renderConnectionNodes({
	...
	if (
		!nodeOrigin ||
		!nodeDestiny ||
		typeof nodeOrigin === "string" ||
		typeof nodeDestiny === "string"
	)
		return;
	if (!connection.pointers) {
		...
		connector = OrthogonalConnector.route({
			...
			globalBounds: {
				...
				width: Math.max(
					shapeA.left + shapeA.width + 100,
					shapeB.left + shapeB.width + 100,
				),
				height: Math.max(
					shapeA.top + shapeA.height + 100,
					shapeB.top + shapeB.height + 100,
				),
			},
		});
...
function calcBezier({
	...
	const CX = Number.parseFloat(
		(
			pointX -
			minleft +
			Math.abs(FX - minleft - (pointX - minleft)) * 0.4
		).toFixed(2),
	);
	const CY = Number.parseFloat(pointY.toFixed(2));
	const DX = Number.parseFloat(
		(FX - minleft - Math.abs(FX - minleft - (pointX - minleft)) * 0.4).toFixed(
			2,
		),
	);
...
function cubicN({
	...
	D,
	F,
}: { T: number; A: number; C: number; D: number; F: number }) {
	const t2 = T * T;
	const t3 = t2 * T;
	return (
		A +
