...
export function addAnimation({
	...
	for (const connection of node.connections.filter(
		(f) => f.idNodeOrigin === node.id && f.connectorType === conn.type && f.connectorName === conn.name
	)) {
...
export function renderConnectionNodes({
	...
	if (!connection.pointers) {
		...
		const indexOutput = nodeOrigin.info.connectors.outputs.findIndex((f) => f.name === connection.connectorName)
		const indexInput = nodeDestiny.info.connectors.inputs.findIndex((f) => f.name === connection.connectorDestinyName)
