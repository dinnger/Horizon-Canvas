...
export class Canvas {
	...
	init() {
		...
		this.addImageProcess(
			this.theme === "light" ? pattern_light : pattern_dark,
		).then((img) => {
	...
	private eventMouseMove = (e: MouseEvent) => {
		...
		this.canvasRelativePos = {
			x: Number.parseFloat(
				((x - this.canvasTranslate.x) / this.canvasFactor).toFixed(2),
			),
			y: Number.parseFloat(
				((y - this.canvasTranslate.y) / this.canvasFactor).toFixed(2),
			),
		};
